...
import { PersonSearch } from '../../src/types.js';

const router = Router();
const collectionName = 'people_search';
...
router.post('/', async (req: Request, res: Response) => {
  try {
    const data = req.body as Partial<PersonSearch>;
    
    if (!data.name || !data.status || !data.registeredBy) {
      res.status(400).json({ error: 'Missing required fields' });
      return;
    }
    
    data.createdAt = data.createdAt || Date.now();
    data.name_slug = data.name.toLowerCase().replace(/[^a-z0-9]/g, '-');

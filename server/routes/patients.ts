...
import { HospitalPatient } from '../../src/types.js';

const router = Router();
const collectionName = 'hospital_patients';
...
router.post('/', async (req: Request, res: Response) => {
  try {
    const data = req.body as Partial<HospitalPatient>;
    
    if (!data.fullName || !data.ci || !data.hospitalName || !data.status || !data.reportedBy) {
